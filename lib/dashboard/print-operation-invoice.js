...
export function buildOperationInvoiceHtml(op, options) {
  const rawBranch = (options?.branchLabel ?? "").trim() || "—";
  const branchLabel = rawBranch.includes("المصطفى") ? "المصطفى" : rawBranch;
  ...
  <div class="sheet">
    ...
    <table>
      <tbody>
        ...
        <tr><th>الوسيلة</th><td>${sourcePhone || "—"}</td></tr>
        <tr><th>المبلغ</th><td>${valStr}</td></tr>
        <tr><th>الرسوم</th><td>${comStr}</td></tr>
    ...
    <div class="customer-note">عميلنا العزيز برجاء عد النقدية قبل الخروج من المحل</div>
  </div>
</body>
</html>`;
}
 ...
 * @param {{ branchLabel?: string }} [options]
 */
export function buildOperationWhatsAppText(op, options) {
  const rawBranch = (options?.branchLabel ?? "").trim() || "الفرع";
  const branchLabel = rawBranch.includes("المصطفى") ? "المصطفى" : rawBranch;
  const created = opCreatedAtToDate(op.createdAt);
  const dateStr =
    created.getTime() === 0
 ...
 */
export async function generateInvoicePdfBlob(op, options) {
  const html = buildOperationInvoiceHtml(op, options ?? {});

  // Force print styles for clean PDF rendering
  ...
  try {
    ...
    const sheet = bodyEl.querySelector(".sheet");
    const target = /** @type {HTMLElement} */ (sheet || bodyEl);
