 ...
 * @param {string} collectionName
 */
export function computeLineActivationAlerts(data, id, collectionName) {
  const raw = typeof data.activationDate === "string" ? data.activationDate.trim() : "";
  if (!raw) return null;

  const parts = raw.split("-");
  if (parts.length !== 3) return null;
  const activation = new Date(Number(parts[0]), Number(parts[1]) - 1, Number(parts[2]));
  if (Number.isNaN(activation.getTime())) return null;

  const expiry = new Date(activation);
  expiry.setMonth(expiry.getMonth() + ACTIVATION_MONTHS);
  ...
  return {
    ...
    name,
    activationDate: raw,
    expiredSince: expiry.toISOString().slice(0, 10),
  };
}
...
export async function fetchShopLimitAlerts(shop) {
  ...
  for (const collName of LINE_COLLECTIONS) {
    const q = query(collection(db, collName), where("shop", "==", s), limit(500));
    const snap = await getDocs(q);
    for (const d of snap.docs) {
      const data = /** @type {Record<string, unknown>} */ (d.data());
      const item = computeLineLimitAlerts(data, d.id, collName);
...
/**
 * @param {string} shop
 * @returns {Promise<Array<NonNullable<ReturnType<typeof computeLineActivationAlerts>>>>}
 */
export async function fetchShopActivationAlerts(shop) {
  const s = shop.trim();
  if (!s) return [];

  /** @type {Array<NonNullable<ReturnType<typeof computeLineActivationAlerts>>>} */
  const all = [];
  ...
  for (const collName of LINE_COLLECTIONS) {
    ...
    for (const d of snap.docs) {
      const data = /** @type {Record<string, unknown>} */ (d.data());
      const item = computeLineActivationAlerts(data, d.id, collName);
      if (item) all.push(item);
    }
  }
