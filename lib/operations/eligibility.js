...
export function analyzeOperation(input) {
  ...
  if (kind === SOURCE_KIND.MACHINE) {
    ...
    if (opType === OPERATION_TYPE.DEPOSIT) {
      ...
      if (!balanceOk) messages.push("الرصيد غير كافٍ لتغطية المبلغ والرسوم.");
    }
    return {
      balanceOk,
      dailyOk: true,
  ...
  if (opType === OPERATION_TYPE.WITHDRAW || opType === OPERATION_TYPE.LIQUIDATION) {
    /** سحب خط: الرسوم تُسجّل فقط ولا تُخصم من الرصيد؛ الليميت = متبقي على المستند. */
    const need = amt;
...
/**
 * اقتراحات تعديل المبلغ/الرسوم لتصبح العملية مسموحة (بدون تنفيذ تلقائي).
 * @param {{
 *   sourceKind: import("./constants").SourceKind;
 *   sourceRow: Record<string, unknown>;
 *   operationType: import("./constants").OperationType;
  ...
  if (kind === SOURCE_KIND.MACHINE) {
    ...
    if (maxCom < com) {
      suggestions.push({
        ...
        label: `اضبط الرسوم إلى ${maxCom} (أقصى ما يسمح به الرصيد مع المبلغ الحالي).`,
    ...
    if (maxAmt < amt) {
      if (maxAmt > 0) {
        suggestions.push({
          id: "machine-amount",
          label: `اضبط المبلغ إلى ${maxAmt} (أقصى ما يسمح به الرصيد مع الرسوم الحالية).`,
          patch: { amount: maxAmt },
        });
      }
    }
  ...
  if (opType === OPERATION_TYPE.WITHDRAW) {
    ...
    if (maxAmt < amt && maxAmt > 0) {
      suggestions.push({
        ...
        label: `اضبط المبلغ إلى ${maxAmt} (متبقي سحب يومي/شهري — الرسوم لا تؤثر على الخط).`,
        patch: { amount: maxAmt },
      });
    }
    return suggestions;
