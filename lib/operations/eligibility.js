...
export function aggregateUsageFromOperations(operations, ctx) {
  ...
  for (const op of operations) {
    ...
    if (type === OPERATION_TYPE.WITHDRAW || type === OPERATION_TYPE.LIQUIDATION || type === OPERATION_TYPE.SMALL_CHANGE_CARDS || type === OPERATION_TYPE.BILLS || type === OPERATION_TYPE.OTHER) {
      const vol = amt;
...
export function analyzeOperation(input) {
  ...
  if (kind === SOURCE_KIND.MACHINE) {
    ...
    if (opType === OPERATION_TYPE.DEPOSIT) {
      balanceOk = balance > 0;
      if (!balanceOk) messages.push("الرصيد صفر، لا يمكن الإيداع على هذه الوسيلة.");
      ...
      if (!balanceOk) messages.push("الرصيد غير كافٍ لتغطية المبلغ والأرباح.");
    }
    return {
      balanceOk,
      dailyOk: true,
  ...
  if (opType === OPERATION_TYPE.WITHDRAW || opType === OPERATION_TYPE.LIQUIDATION) {
    /** سحب خط: الأرباح تُسجّل فقط ولا تُخصم من الرصيد؛ الليميت = متبقي على المستند. */
    const need = amt;

    if (rem.remDailyWithdraw > 0 && rem.remDailyWithdraw < need) {
      messages.push("تجاوز المتبقي اليومي للسحب.");
    }
    if (rem.remMonthlyWithdraw > 0 && rem.remMonthlyWithdraw < need) {
      messages.push("تجاوز المتبقي الشهري للسحب.");
    }
  } else if (opType === OPERATION_TYPE.EXTERNAL) {
    // معاملة خارجية: لا تؤثر على الليميت ولا على رصيد الخط
  } else if (opType === OPERATION_TYPE.DEPOSIT) {
    balanceOk = balance >= amt;
    if (!balanceOk) messages.push("الرصيد غير كافٍ للإيداع.");
    if (rem.remDailyDeposit > 0 && rem.remDailyDeposit < amt) {
      messages.push("تجاوز المتبقي اليومي للإيداع.");
    }
    if (rem.remMonthlyDeposit > 0 && rem.remMonthlyDeposit < amt) {
      messages.push("تجاوز المتبقي الشهري للإيداع.");
    }
  }
...
export function computeLineLimitRemainderSnapshots(input) {
  ...
  if (input.operationType === OPERATION_TYPE.WITHDRAW || input.operationType === OPERATION_TYPE.LIQUIDATION || input.operationType === OPERATION_TYPE.EXTERNAL) {
    const need = amt;
...
/**
 * اقتراحات تعديل المبلغ/الأرباح لتصبح العملية مسموحة (بدون تنفيذ تلقائي).
 * @param {{
 *   sourceKind: import("./constants").SourceKind;
 *   sourceRow: Record<string, unknown>;
 *   operationType: import("./constants").OperationType;
...
export function buildOperationSuggestions(input) {
  ...
  if (kind === SOURCE_KIND.MACHINE) {
    ...
    if (maxCom < com) {
      suggestions.push({
        ...
        label: `اضبط الأرباح إلى ${maxCom} (أقصى ما يسمح به الرصيد مع المبلغ الحالي).`,
    ...
    if (maxAmt < amt) {
      if (maxAmt > 0) {
        suggestions.push({
          id: "machine-amount",
          label: `اضبط المبلغ إلى ${maxAmt} (أقصى ما يسمح به الرصيد مع الأرباح الحالية).`,
          patch: { amount: maxAmt },
        });
      }
    }
  ...
  if (opType === OPERATION_TYPE.WITHDRAW) {
    ...
    if (!Number.isFinite(maxAmt)) return suggestions;
    maxAmt = round2(Math.max(0, maxAmt));

    if (maxAmt < amt && maxAmt > 0) {
      suggestions.push({
        id: "line-withdraw-amount",
        label: `اضبط المبلغ إلى ${maxAmt} (متبقي سحب يومي/شهري — الأرباح لا تؤثر على الخط).`,
        patch: { amount: maxAmt },
      });
    }
    return suggestions;
  ...
  if (opType === OPERATION_TYPE.DEPOSIT) {
    ...
    if (!Number.isFinite(maxDep) || maxDep <= 0) return [];
    const capAmt = round2(Math.max(0, maxDep));
    if (capAmt < amt && capAmt > 0) {
      suggestions.push({
        id: "line-deposit-amount",
        label: `خفّض المبلغ إلى ${capAmt} (رصيد الخط أو متبقي الإيداع اليومي/الشهري لا يسمح بأكثر من ذلك).`,
        patch: { amount: capAmt },
      });
    }
  }
...
/**
 * حجم «الحاجة» لعرض الملاءمة: إيداع = المبلغ؛ سحب خط = المبلغ فقط؛ سحب ماكينة = مبلغ + رسوم.
