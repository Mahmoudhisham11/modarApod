...
/** @typedef {"withdraw" | "deposit" | "balance_transfer" | "small_change_cards" | "bills" | "other" | "cash_addition" | "liquidation" | "external" | "merchant"} OperationType */
...
export const OPERATION_TYPE_LABEL = {
  ...
  liquidation: "تسيل أموال",
  external: "معاملة خارجية",
  merchant: "عملية تاجر",
};

/** @param {SourceKind} kind */
export function operationTypesForSourceKind(kind) {
  const common = [OPERATION_TYPE.LIQUIDATION, OPERATION_TYPE.EXTERNAL, OPERATION_TYPE.MERCHANT];
  if (kind === SOURCE_KIND.TELECOM || kind === SOURCE_KIND.INSTAPAY) {
    return [OPERATION_TYPE.WITHDRAW, OPERATION_TYPE.DEPOSIT, ...common];
  }
  return [
...
export function isMachineDebitOperation(op) {
  return (
    ...
    op === OPERATION_TYPE.LIQUIDATION ||
    op === OPERATION_TYPE.EXTERNAL ||
    op === OPERATION_TYPE.MERCHANT
  );
}
