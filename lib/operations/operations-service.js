...
export async function createOperationWithUpdates(payload) {
  ...
  // EXTERNAL: skip analyzeOperation (limits check), handle manually
  const shouldBypassAnalysis = opType === OPERATION_TYPE.EXTERNAL;
  ...
  } else if (opType === OPERATION_TYPE.WITHDRAW) {
    batch.update(shopRef, { cash: increment(-amt) });
  }
...
export async function deleteOperationWithReversal(params) {
  ...
  await runTransaction(db, async (transaction) => {
    ...
    if (kind === SOURCE_KIND.MACHINE) {
      ...
      if (typeStr !== OPERATION_TYPE.WITHDRAW && typeStr !== OPERATION_TYPE.DEPOSIT && typeStr !== OPERATION_TYPE.LIQUIDATION) {
