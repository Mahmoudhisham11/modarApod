...
export async function createOperationWithUpdates(payload) {
  ...
  const sourceRow = /** @type {Record<string, unknown> | undefined} */ (allSources.find((s) => s.id === sourceId));
  ...
  if (payload.sourceKind === SOURCE_KIND.MACHINE) {
    ...
    const lineUpdate = {
      amount: formatLineAmountString(opType === OPERATION_TYPE.WITHDRAW ? beforeBalance + amt : beforeBalance - amt),
    };
