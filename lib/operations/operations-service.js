...
export async function createOperationWithUpdates(payload) {
  ...
  return runTransaction(db, async (transaction) => {
    ...
    let currentCash = 0;
    if (opType === OPERATION_TYPE.WITHDRAW || opType === OPERATION_TYPE.DEPOSIT) {
    ...
    if (payload.sourceKind === SOURCE_KIND.MACHINE) {
      ...
      const lineUpdate = {
        amount: formatLineAmountString(afterBalance),
      };
      if (opType === OPERATION_TYPE.WITHDRAW) {
    ...
    /** تحديث النقدي (الصندوق): السحب يطرح، الإيداع يضيف */
    if (opType === OPERATION_TYPE.WITHDRAW) {
