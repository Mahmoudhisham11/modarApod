...
export async function createOperationWithUpdates(payload) {
  ...
  const merchantCommission = Number(payload.merchantCommission);
  const merchantCom = Number.isFinite(merchantCommission) && merchantCommission > 0 ? merchantCommission : 0;
  ...
  // EXTERNAL & MERCHANT: skip analyzeOperation (limits check), handle manually
  const shouldBypassAnalysis = opType === OPERATION_TYPE.EXTERNAL || opType === OPERATION_TYPE.MERCHANT;
  ...
  if (sourceRow && payload.sourceKind === SOURCE_KIND.MACHINE) {
    ...
    if (opType === OPERATION_TYPE.DEPOSIT) {
      ...
      const need = amt + com;
      if (beforeBalance < need) throw new Error("الرصيد غير كافٍ للتحويل والأرباح.");
  ...
  const opDoc = {
    ...
    merchantCommission: merchantCom,
  ...
  if (opType === OPERATION_TYPE.LIQUIDATION || opType === OPERATION_TYPE.DEPOSIT) {
    batch.update(shopRef, { cash: increment(amt) });
  } else if (opType === OPERATION_TYPE.WITHDRAW || opType === OPERATION_TYPE.MERCHANT) {
    batch.update(shopRef, { cash: increment(-amt) });
  }
...
export async function deleteOperationWithReversal(params) {
  ...
  await runTransaction(db, async (transaction) => {
    ...
    // EXTERNAL: المعاملة الخارجية مش بتأثر على رصيد ولا نقدي — بنحذف السجل بس
    if (typeStr === OPERATION_TYPE.EXTERNAL) {
      transaction.delete(opRef);
      return;
    }
    ...
    if (kind === SOURCE_KIND.MACHINE) {
      ...
      if (typeStr !== OPERATION_TYPE.WITHDRAW && typeStr !== OPERATION_TYPE.DEPOSIT && typeStr !== OPERATION_TYPE.LIQUIDATION && typeStr !== OPERATION_TYPE.MERCHANT) {
        throw new Error("حذف هذا النوع من العمليات على الخط غير مدعوم.");
      }
      const beforeBalance = parseLineAmount(sourceRow);
      let afterBalance = beforeBalance;
      if (typeStr === OPERATION_TYPE.WITHDRAW || typeStr === OPERATION_TYPE.MERCHANT) {
        /** عكس السحب: ينقص رصيد الخط. */
        afterBalance = beforeBalance - amt;
      } else if (typeStr === OPERATION_TYPE.LIQUIDATION) {
        /** عكس التسيل: يزيد رصيد الخط. */
        afterBalance = beforeBalance + amt;
      } else {
      ...
      if (lim && typeof lim === "object") {
        ...
        // LIQUIDATION القديمة (قبل إضافة تأثير الليميت): لا يوجد limitAdjustment، عكس يدوي
        // LIQUIDATIONS الجديدة لها limitAdjustment ويتم عكسها تلقائيًا أعلاه
        if (typeStr === OPERATION_TYPE.LIQUIDATION) {
          if ("dailyWithdraw" in sourceRow && sourceRow.dailyWithdraw != null && sourceRow.dailyWithdraw !== "") {
            const cur = parseFiniteNumberOrZero(asString(sourceRow.dailyWithdraw));
            lineUpdate.dailyWithdraw = round2(cur + amt);
          }
    ...
    if (typeStr === OPERATION_TYPE.WITHDRAW || typeStr === OPERATION_TYPE.MERCHANT) {
      transaction.set(cRef, { cash: currentCash + amt }, { merge: true });
    } else if (typeStr === OPERATION_TYPE.LIQUIDATION || typeStr === OPERATION_TYPE.DEPOSIT) {
      transaction.set(cRef, { cash: Math.max(0, currentCash - amt) }, { merge: true });
    }
