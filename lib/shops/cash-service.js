...
export async function fetchShopCapitalData(shop) {
  ...
  try {
    ...
    for (const colName of collections) {
      ...
      const q = query(colRef, where("shop", "==", s), limit(200));
...
export async function createCashAdditionOperation(payload) {
  ...
  const direction = payload.direction === "subtract" ? "subtract" : "add";
  ...
  return runTransaction(db, async (transaction) => {
    ...
    const newCash = direction === "subtract" ? oldCash - amt : oldCash + amt;

    if (newCash < 0) {
      throw new Error("النقدي لا يكفي لإتمام السحب.");
    }
    ...
    const opDoc = {
      ...
      commation: 0,
      notes: `${direction === "subtract" ? "سحب" : "إيداع"} نقدي${note ? ` — ${note}` : ""}`,
      beforeCash: oldCash,
      afterCash: newCash,
      userName,
      createdBy,
