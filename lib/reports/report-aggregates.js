...
export function filterOperationsByPeriod(items, opts) {
  ...
  // Use explicit date inputs if provided (regardless of preset)
  if (opts.dateFrom || opts.dateTo) {
    const rangeStart = opts.dateFrom ? startOfLocalDayFromInput(opts.dateFrom) : null;
    const rangeEnd = opts.dateTo ? endOfLocalDayFromInput(opts.dateTo) : endOfLocalDay(now);
...
function reportTypeLabel(row) {
  ...
  const label = OPERATION_TYPE_LABEL[/** @type {keyof typeof OPERATION_TYPE_LABEL} */ (t)];
  return label || t.trim() || "—";
}
